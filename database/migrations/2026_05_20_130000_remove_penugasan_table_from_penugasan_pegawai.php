...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (Schema::hasTable('penugasan_pegawai')) {
            ...
            if (!$this->indexExists('penugasan_pegawai', 'penugasan_pegawai_kegiatan_pegawai_index')) {
                Schema::table('penugasan_pegawai', function (Blueprint $table) {
    ...
    public function down(): void
    {
        ...
        if (Schema::hasTable('penugasan_pegawai') && Schema::hasColumn('penugasan_pegawai', 'id_kegiatan')) {
            DB::statement('
                INSERT INTO penugasan (id_kegiatan)
                SELECT DISTINCT id_kegiatan
                FROM penugasan_pegawai
                WHERE id_kegiatan IS NOT NULL
                AND id_kegiatan NOT IN (
                    SELECT id_kegiatan FROM penugasan WHERE id_kegiatan IS NOT NULL
                )
            ');

            Schema::table('penugasan_pegawai', function (Blueprint $table) {
                $table->unsignedBigInteger('id_penugasan')->nullable()->after('id');
            });

            DB::statement('
                UPDATE penugasan_pegawai pp
                JOIN penugasan p ON p.id_kegiatan = pp.id_kegiatan
                SET pp.id_penugasan = p.id_penugasan
            ');
